...
At each cooling detuning, 25 independently seeded realizations of a preloaded 25-atom Rb-87 cloud are evolved in the fixed six-beam MOT using the repumper-enabled 24-state adiabatic population-rate equations and Langevin recoil diffusion. Each realization's reported final temperature is the time average over the final 5 ms of the unbiased, instantaneous-center-of-mass-subtracted three-dimensional velocity variance; the plotted point is the arithmetic mean of the 25 realization temperatures. Only the cooling detuning Delta=n Gamma changes between points.
...
Instead, each of the 25 independent realizations is a randomly
sampled 25-atom preloaded cloud. A plotted estimate is
...
- Detuning grid: 23 specified values spanning n=-6 to n=-0.5, with Delta=n Gamma and Gamma/(2 pi)=6.07 MHz.
...
- Across the saved detuning grid, this reference spans 304.766 to 1075.72 microkelvin and is plotted as a curve, not a constant line.

The 25 random initial phase-space clouds are drawn once and reused at every
...
the arithmetic mean of the 25 realization temperatures. Explicit
asymmetric error bars and the shaded band show the two-sided 95% Student-t
interval across the 25 independent cloud realizations; one SEM is
...
point is marked valid only if all 25 cloud estimates pass. Hollow red circles
...
uncertainty interval is a 95% Student-t interval across the 25 realization
fractions, which preserves the ensemble clustering. A pooled 95% Wilson
interval across all 625 trajectories is also saved as a secondary diagnostic.
...
captured. This run uses 625 trajectories per detuning and reports an interval.
