"""Sweep horizontal pMOT geometries for a 58 deg objective-clearance beam.

This sweep constrains both 1530 and 780 paths to enter at 58 deg relative to
the glass-cell normal, with a 12.7 mm external collimated beam diameter.
"""
...
def evaluate_lens(lens: LensSpec) -> dict[str, object]:
    ...
    l1_collision = mount_collides_with_simplified_cell(
        geometry,
        solution["z_lens1_mount_left"],
        solution["z_lens1_mount_right"],
        lens.mount_half_height,
    )
    l2_collision = mount_collides_with_simplified_cell(
        geometry,
        solution["z_lens2_mount_left"],
        solution["z_lens2_mount_right"],
        lens.mount_half_height,
    )
    return {
        "lens": lens,
        ...
        "l1_collision": l1_collision,
        "l2_collision": l2_collision,
        "allowed": not (l1_collision or l2_collision),
        "score": abs(degrees(solution["full_angle_avg"]) - 9.0),
    }


def simplified_cell_polygons(geometry: RoundTripGeometry) -> list[list[tuple[float, float]]]:
    """Return the physical simplified cell polygons in z/y meters."""
    ...
    # Approximate the glass as convex wall/collar blocks instead of as a filled
    # outer body with a white aperture. This matches the drawn physical solids.
    rects = [
        (u_inner_max, u_max, v_inner_min, v_inner_max),  # closed end
        (u_min, u_max, v_inner_max, v_max),  # top wall
        (u_min, u_max, v_min, v_inner_min),  # bottom wall
        (u_col_min, u_col_max, v_col_min, v_inner_min),  # collar bottom
        (u_col_min, u_col_max, v_inner_max, v_col_max),  # collar top
    ]
    return [cell_rect_polygon(geometry, *rect) for rect in rects]


def cell_rect_polygon(
    geometry: RoundTripGeometry,
    u0: float,
    u1: float,
    v0: float,
    ...
    for u, v in [(u0, v0), (u0, v1), (u1, v1), (u1, v0)]:
        z = u * cos(cell_axis_angle) + v * cos(normal_angle)
        y = u * sin(cell_axis_angle) + v * sin(normal_angle)
        points.append((z, y))
    return points


def mount_rect_polygon(z_left: float, z_right: float, y_half: float) -> list[tuple[float, float]]:
    return [
        (z_left, -y_half),
        (z_left, y_half),
        (z_right, y_half),
...
def mount_collides_with_simplified_cell(
    geometry: RoundTripGeometry,
    z_left: float,
    z_right: float,
    y_half: float,
) -> bool:
    mount = mount_rect_polygon(z_left, z_right, y_half)
    return any(polygons_intersect(mount, cell_poly) for cell_poly in simplified_cell_polygons(geometry))


def polygons_intersect(poly_a: list[tuple[float, float]], poly_b: list[tuple[float, float]]) -> bool:
    """Convex polygon intersection via the separating-axis theorem."""
...
def print_table(results: list[dict[str, object]]) -> None:
    ...
    print(f"Cell AOI = {CELL_AOI_DEG:.1f} deg; target foci = +/-10 mm; mirror gap = 40 mm")
    print()
    header = (
        "lens                 f(mm)  half(deg) full(deg) "
        "L1(mm)    L2(mm)    mirror(mm)  waist1(um) waist2(um) "
        "L1 env(mm)              L2 env(mm)             ok?"
    )
    print(header)
    print("-" * len(header))
    for result in results:
        ...
        print(
            ...
            f"  {'yes' if result['allowed'] else 'no'}"
        )
...
def main() -> None:
    ...
    allowed = [
        ...
        for result in results
        if result["allowed"]
        and result["solution"]["full_angle_avg"] <= 9.5 * 3.141592653589793 / 180.0
    ]
    ...
    output_path = Path(__file__).with_name("Horizontal58degObjectiveClearance_best.png")
