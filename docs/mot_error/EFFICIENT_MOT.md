...
> **ARCHIVED AND PHYSICALLY INVALID:** the implementation specified here uses
> a saturated two-level scattering expression as an elementary bidirectional
> rate in a multilevel population matrix. This document is retained only as a
> record of the superseded model. It is not authoritative for the replacement
> solver in `src/pmot/mot_multilevel`.
