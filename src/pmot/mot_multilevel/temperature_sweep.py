...
Temperature is an ensemble property, so this study evolves ten independently
sampled, preloaded 25-atom clouds at every detuning.  The external dynamics use
...
def detuning_n_grid() -> np.ndarray:
    """Return a mutable copy of the authoritative 25-point Delta/Gamma grid."""
...
def _detuning_invariant_payload(detuning_n: float) -> dict[str, object]:
    config, apparatus, beams = build_temperature_sweep_configuration(detuning_n)
...
def verify_only_cooling_detuning_changes(
    ...
    if not detuning_values:
        raise ValueError("detuning_values must not be empty")
    reference_payload = _detuning_invariant_payload(float(detuning_values[0]))
    reference_json = json.dumps(reference_payload, sort_keys=True, separators=(",", ":"))
    base = default_multilevel_mot_config()
    for value in detuning_values:
        config, apparatus, beams = build_temperature_sweep_configuration(float(value))
        ...
        candidate_json = json.dumps(
            _detuning_invariant_payload(float(value)),
            sort_keys=True,
            separators=(",", ":"),
        )
    ...
    return {
        "passed": True,
        "point_count": len(detuning_values),
...
def _temperature_sweep_worker(
    ...
    config, beam_tuple, model = _worker_context(payload.detuning_n)
...
def _mean_sem_t_interval(
    ...
    *,
    ...
    critical = _T_975_BY_DF.get(len(finite) - 1, 1.959963984540054)
...
def _metadata_payload(
    *,
    ...
    return {
        ...
        "ensemble_realization_count_per_point": REQUIRED_ENSEMBLE_REALIZATION_COUNT,
        "atoms_per_ensemble": REQUIRED_ATOMS_PER_ENSEMBLE,
        "trajectory_count_per_point": REQUIRED_TRAJECTORY_COUNT,
        ...
        "study_design_rationale": (
            "Temperature is estimated from ten independently sampled preloaded 25-atom "
            "clouds. Incident capture disks are not used because their required launch "
            "speed would add a second varied physical input."
        ),
        ...
        "reported_point_definition": (
            "arithmetic mean of the ten independently calculated realization plateau "
            "temperatures"
        ),
        "equilibrium_quality_definition": (
            "valid only when all ten clouds meet the minimum final-core survivor "
        ...
        "temperature_uncertainty_definition": (
            "one SEM and a two-sided 95% Student-t interval across the ten independent "
            "ensemble-realization temperatures"
        ),
        "survivor_fraction_uncertainty_definition": (
            "primary: 95% Student-t interval across ten 25-atom realization fractions; "
            "secondary: pooled two-sided 95% Wilson score interval over 250 trajectories"
        ),
        ...
        "common_initial_condition_strategy": (
            "ten independent initial phase-space clouds drawn once and reused exactly at "
            "every detuning"
        ),
        ...
        "apparatus_config_repump_note": (
            "The nested resume-signature apparatus_config is the generic shared "
            "apparatus record. The simulated repumper power is the effective value "
            "above and in multilevel_config (0.1 mW per beam), not the generic "
            "apparatus repump entry."
        ),
...
def plot_temperature_vs_detuning(
    ...
    *,
    ...
    """Plot the ten-cloud mean temperature and the separate survivor fraction."""
...
def run_temperature_detuning_sweep(
    *,
    ...
    minimum_survivor_count: int = DEFAULT_MINIMUM_SURVIVORS_PER_ENSEMBLE,
    stationarity_limit: float = DEFAULT_STATIONARITY_LIMIT,
) -> dict[str, object]:
    """Run, checkpoint, resume, and plot the 10x25 temperature study."""

    if ensemble_realization_count != REQUIRED_ENSEMBLE_REALIZATION_COUNT:
        raise ValueError(
            "the production temperature sweep requires exactly 10 ensemble realizations"
        )
    if atoms_per_ensemble != REQUIRED_ATOMS_PER_ENSEMBLE:
        raise ValueError(
            "the production temperature sweep requires exactly 25 atoms per ensemble"
        )
    ...
    if minimum_survivor_count < 2 or minimum_survivor_count > atoms_per_ensemble:
        raise ValueError("minimum survivor count must be between 2 and 25")
    ...
    sweep_name = "temperature_vs_detuning_10x25_preloaded_ensembles"
    ...
    expected_index = {
        _detuning_key(value): index for index, value in enumerate(DETUNING_N_VALUES)
    }
    ...
    total_new_trajectories = len(remaining) * REQUIRED_TRAJECTORY_COUNT
    completed_new_trajectories = 0
    print(
        f"[temperature-sweep] points={len(DETUNING_N_VALUES)}; "
        f"resumed={resumed_point_count}; remaining={len(remaining)}; "
        f"ensembles/point={ensemble_realization_count}; "
        f"atoms/ensemble={atoms_per_ensemble}; trajectories/point={REQUIRED_TRAJECTORY_COUNT}; "
    ...
    if remaining:
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            for point_index, detuning_n in remaining:
                ...
                print(
                    f"[temperature-sweep] point {point_index+1}/{len(DETUNING_N_VALUES)} "
                    f"start: n={detuning_n:+.2f}, "
                    f"Delta/(2pi)={detuning_n*gamma_hz/1e6:+.3f} MHz; "
                    "submitting 10 x 25 trajectories",
                    flush=True,
                )
                payloads = [
                ...
                for future in as_completed(future_to_identity):
                    ...
                    if (
                        ...
                        or completed_point_trajectories % 10 == 0
                        or outcome != "ok"
                    ):
                        print(
                            f"[temperature-sweep] point {point_index+1}/{len(DETUNING_N_VALUES)} "
                            f"n={detuning_n:+.2f}; trajectories "
                            f"{completed_point_trajectories}/{REQUIRED_TRAJECTORY_COUNT}; "
                            f"ensemble {ensemble_index+1}/10 atom {atom_index+1}/25; "
                ...
                plot_temperature_vs_detuning(rows, plot_path)
                ...
                print(
                    ...
                    f"cloud temperatures={row['temperature_ensemble_count']}/10; "
                    f"survivors={row['trapped_atom_count']}/{REQUIRED_TRAJECTORY_COUNT}; "
    ...
    print(
        f"[temperature-sweep] {final_status}: {len(rows)}/{len(DETUNING_N_VALUES)} "
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the 25-point 24-state multilevel MOT temperature sweep with "
            "10 independently sampled 25-atom preloaded clouds per detuning"
        )
    )
    parser.add_argument(
        ...
        help="must be exactly 10 for the production study",
    )
    parser.add_argument(
        "--atoms-per-ensemble",
        type=int,
        default=REQUIRED_ATOMS_PER_ENSEMBLE,
        help="must be exactly 25 for the production study",
    )
