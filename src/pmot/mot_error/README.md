# Archived erroneous multilevel MOT package

> **Archive warning:** this implementation is preserved for provenance and
> regression only. Its per-transition rate is a saturated two-level scattering
> formula inserted into a multilevel population-rate matrix. That closure is
> physically invalid for the intended multilevel MOT, so its force,
> capture/loading, trajectory, and temperature results must not be used as
> physical predictions. The clean rebuild lives in `src/pmot/mot_multilevel`.

The archived full-MOT trajectory algorithm is the efficient adiabatic-elimination
population-rate model specified by `docs/mot_error/EFFICIENT_MOT.md`. It retains the full
...
/home/ajrosy/pMOT_MonteCarlo/.venv_pMOT_MC/bin/python \
  -m pmot.mot_error.rate_run --duration-ms 25 --dt-us 5
```
...
/home/ajrosy/pMOT_MonteCarlo/.venv_pMOT_MC/bin/python \
  -m pmot.mot_error.rate_capture --disc-count 50 --points-per-disc 25 --workers 8
```
...
OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
/home/ajrosy/pMOT_MonteCarlo/.venv_pMOT_MC/bin/python \
  -m pmot.mot_error.sampling_disc_radius_study --workers 24
```
...
OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
/home/ajrosy/pMOT_MonteCarlo/.venv_pMOT_MC/bin/python \
  -m pmot.mot_error.refined_relationship_campaign campaign --workers 24
```
...
`notebooks/mot_error/full_mot_single_trajectory.ipynb`. It adapts the
...
from pmot.mot_error.jump_visualization import create_hyperfine_jump_animation
